...
# Which guard overall was asleep on a same minute without considering cumulative
# minutes asleep? Repeat the previous logic but instead of first finding the most
# sleepy guard, just do it for every guard finding the individual minute with the
# highest count overall.

# Tuple consist of the minute, and the count of times asleep in that minute, and
# the ID of the guard who did it.
guard_most_minute_asleep = (-1, -1, -1)
for sleepy_guard in guards_to_minutes_asleep.keys():
    for minute, count in guards_to_minutes_asleep[sleepy_guard].items():
        max_count = guard_most_minute_asleep[1]
        if count > max_count:
            guard_most_minute_asleep = (minute, count, sleepy_guard)

# Multiply the ID of the guard by the individual minute they were asleep the most.
# Result: 58559.
print(guard_most_minute_asleep[0] * guard_most_minute_asleep[2])
