...
foreach my $memory_row (@{ $memory }) {
    ...
    # Now that we know these banlks have infinite loops, we want the
    # number of iterations from the first time the first looping
    # position was found, and the time it is found again.
    my $duplicate_state = undef;
    ...
    while (1) {
        ...
        last if ($duplicate_state // "") eq $current_state;
        # Store the first found duplicate state.
        if (defined $seen_before->{$current_state} && !(defined $duplicate_state)) {
            $duplicate_state = $current_state;
            $iterations = 0;
        };
